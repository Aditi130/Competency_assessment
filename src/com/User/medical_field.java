...
/**
 * Servlet implementation class medical_field
 */
@WebServlet("/medical_field")
public class medical_field extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public medical_field() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			st.executeUpdate("delete from tblresult where username='" + emailid
					+ "'");
			question_and_answer.Q1(con, id1, Q1, opt1, username, Exam_Type);
			question_and_answer.Q2(con, id2, Q2, opt2, username, Exam_Type);
			question_and_answer.Q3(con, id3, Q3, opt3, username, Exam_Type);
			question_and_answer.Q4(con, id4, Q4, opt4, username, Exam_Type);
			question_and_answer.Q5(con, id5, Q5, opt5, username, Exam_Type);
			question_and_answer.Q6(con, id6, Q6, opt6, username, Exam_Type);
			question_and_answer.Q7(con, id7, Q7, opt7, username, Exam_Type);
			question_and_answer.Q8(con, id8, Q8, opt8, username, Exam_Type);
			question_and_answer.Q9(con, id9, Q9, opt9, username, Exam_Type);
			question_and_answer.Q10(con, id10, Q10, opt10, username, Exam_Type);

			Statement st1 = con.createStatement();
			ResultSet rs = st
					.executeQuery("select sum(marks) from tblresult where username='"
							+ emailid + "' and Exam_Type='" + Exam_Type + "'");
			if (rs.next()) {
				ResultSet rs1 = st1
						.executeQuery("select * from smarks where semail='"
								+ emailid + "' and Exam_Type='" + Exam_Type + "'");
				if (rs1.next())
				{
					st1.executeUpdate("update smarks set Total_Marks='"+ rs.getInt(1)+ "' where semail='"
								+ emailid + "' and Exam_Type='" + Exam_Type + "'");
				}
				else{
				
				st1.executeUpdate("insert into smarks (semail,Exam_Type,Total_Marks) values('"
						+ emailid
						+ "','"
						+ Exam_Type
						+ "','"
			...
			RequestDispatcher rd = request
					.getRequestDispatcher("/Student_OtherExam_Show_Drives.jsp");
